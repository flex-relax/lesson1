# импортируем модуль времени и инициализируем пустой список для заметок
import time
note_list =[]

# def finding_duplicate_names(note_list):
#     for el in note_list:
#         if note_name in note_list:
# Задаем функцию валидатор на проверку пустоты строк, используя флаг - булевое значение на выходе
...
def create_note(username, note_name, note_description, note_status, note_create_date, note_deadline):

    note_dict = {'Имя пользователя': username, 'Наименование заметки': note_name,'Описание заметки':note_description,
                 'Статус заметки': note_status, 'Дата создания заметки': note_create_date,'Дедлайн':note_deadline}
...
# Ввод дат проверяем на наличие ошибок формата через try except. при ошибке формата даты итерация пропускается и цикл создания заметки запускается заново
    ...
    flag = check_empty_string(username, note_name, note_description, note_status, note_create_date, note_deadline)
    if flag == False:
...
# Запускаем функцию create_note - создаем заметку
    create_note(username, note_name, note_description, note_status, note_create_date, note_deadline)
# Присваиваем значение функции переменной answer_flag. Запрашиваем у пользователя продолжить ли заполнение заметок, при слове нет программа останавливается
